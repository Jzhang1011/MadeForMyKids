...
(function () {
  ...
  function setTitle(piece) {
    const siteName = (state.site && state.site.name) || "Watch to Grow";
    document.title = piece
      ? `${piece} · ${siteName} · MadeForMyKids`
      : `${siteName} · MadeForMyKids`;
  }

  function ensureActivityToolbar(route) {
    const root = $("[data-app]");
    if (!root) return;
    let bar = $("[data-movie-toolbar]");
    if (!bar) {
      bar = document.createElement("div");
      bar.className = "movie-activity-toolbar";
      bar.setAttribute("data-movie-toolbar", "");
      root.parentNode.insertBefore(bar, root);
    }
    const q = esc(route.view === "search" ? (route.q || "") : "");
    const siteName = esc((state.site && state.site.name) || "Watch to Grow");
    bar.innerHTML = `<div class="wrap nav movie-toolbar-inner">
      <a class="brand" href="#/"><strong>${siteName}</strong><span>Watch to Grow · Movie learning</span></a>
      <nav class="nav-links" aria-label="Watch to Grow">
        <a href="#/">Needs</a>
        <a href="#/">Jobs</a>
    ...
    const form = $("[data-nav-search]", bar);
    if (form) {
  ...
  function renderChrome(route) {
    // Site chrome is now <mfk-header> / <mfk-footer>. Legacy header.site / footer.site
    // are optional; skip injection when absent so we never crash.
