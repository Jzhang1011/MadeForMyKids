...
(function () {
  ...
  function renderHome(root) {
    ...
    const jobDoors = (state.site.professions || []).map((job) => {
      const n = withProfession(job.name).length;
      return `<a class="door" href="${professionHref(job.slug)}">
        <h2>${esc(job.name)}</h2>
        <p>${esc(job.look)}</p>
        <div class="count">${n} titles</div>
      </a>`;
    }).join("");
    ...
    root.innerHTML = `
      ...
      <div class="section-head">
        <h2>Explore a profession</h2>
        <a href="#/profession">${jobbed} titles tagged with a job →</a>
      </div>
      <p class="lede" style="margin-top:-8px;margin-bottom:16px">This is where most of the catalog lives. Lawyer, doctor, chef, analyst — tap a job.</p>
      <section class="doors">${jobDoors}</section>
      <div class="section-head">
        <h2>Or pick a quality to grow</h2>
      </div>
  ...
  function renderProfession(root, route) {
    ...
    if (!job) {
      ...
      const doors = jobs.map((p) => {
        const n = withProfession(p.name).length;
        return `<a class="door" href="${professionHref(p.slug)}">
          <h2>${esc(p.name)}</h2>
          <p>${esc(p.look)}</p>
          <div class="count">${n} titles</div>
        </a>`;
      ...
      root.innerHTML = `
        <section class="page-hero">
          <p class="kicker">🧭 Workplace field trips</p>
          <h1>What job should they see?</h1>
          <p class="lede">Movies and series as a look at real work. Adult shows stay labeled 16+. Billions is a warning about finance culture, not a brochure.</p>
        </section>
